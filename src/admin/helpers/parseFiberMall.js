...
// parseMetaData - парсить мета-дані (title, keywords, description) з HTML сторінок
const parseMetaData = async () => {
    try {
            ...
            try {
                ...
                for (const link of productLinks) {
                    ...
                    try {
                        ...
                        if (pageSku && String(pageSku) === String(sku)) {
                            // Отримуємо мета-дані
                            const metaData = await productPage.evaluate(() => {
                                const title = document.querySelector('title')?.textContent || '';
                                const description = document.querySelector('meta[name="description"]')?.getAttribute('content') || '';
                                const keywords = document.querySelector('meta[name="keywords"]')?.getAttribute('content') || '';

                                return {
                                    title: title.replace(/\s*\|\s*FiberMall\s*$/i, '').trim(),
                                    description,
                                    keywords,
                                };
                            });
                ...
                if (!found) {
                    status = 'not found';
                    message = message || 'Не знайдено або мета-дані відсутні';
                }
        ...
        await browser.disconnect();
        await fs.writeFile('./meta_data.json', JSON.stringify(results, null, 2));
        console.log('✅ Готово! Дані збережено у meta_data.json');
        process.exit(0);
    } catch (error) {
        console.error('Сталася помилка:', error);
    }
...
// parseMetaData().catch(console.error);

    ...
    try {
        ...
        if (IS_CHECKED) {
            await Landing.updateMany(
                { template_pack: TEMPLATE_PAGE_ID },
                { $push: { sections: newSection } },
            );
        }

        const templateOne = await Template.findOne({ _id: TEMPLATE_PAGE_ID });
        const sectionOne = templateOne.sections.find(
            (s) => s._id.toString() === newSection._id.toString(),
        );
...
export const update = async (req, res) => {
    try {
        ...
        if (isChecked) {
            await Landing.updateMany(
                { template_id: templateId, 'sections._id': sectionId },
                { $set: { 'sections.$.title': title } },
            );
        }
...
export const updateStatus = async (req, res) => {
    try {
        ...
        await Template.updateOne(
            { _id: template_id, 'sections._id': section_id },
...
export const updateStatusLendingSection = async (req, res) => {
    try {
        ...
        await Landing.updateMany(
            { template_pack: template_id, 'sections._id': section_id },
            { $set: { 'sections.$.status': status } },
        );
